# FamilyBasket — Full Product Brief for Claude Code
**Version 2.0 — Complete Feature Set — Vercel Ready**

---

## Project Overview

Build a React web application called **FamilyBasket** — a family grocery intelligence tool that scans receipts, scores nutrition, plans batch cooking, generates shopping lists, and learns a family's food preferences over time.
...
## Onboarding — First Run Family Profile

On first launch (after API key setup), show a family profile onboarding screen. This replaces all hardcoded family data and makes the app usable by any family.

Collect the following:

- Family name (e.g. "The Smiths")
- Number of adults
- Number of children (with optional names)
- Weekly grocery budget (£ amount, shown as a range slider with £50–£250)
...
Save this profile to localStorage as `fb_profile`. Use it to dynamically generate the AI system prompt for every Claude API call. No personal data should be hardcoded.
...
Build a function `buildSystemPrompt(profile)` that takes the stored family profile and returns a personalised system prompt string. Example output:

```
You are a family nutrition and grocery advisor.
FAMILY PROFILE:
- Family: The Smiths
...
A dedicated Meals page with three sections: Family Favourites, Adult Meals, and Children's Meals.
...
- Each item shows quantity needed and estimated Aldi price where known
- Gap-filler items clearly labelled as "Nutrition boost — optional"
- Estimated total at the bottom vs. family budget
...
- Full tech stack
- Key file structure
- The family profile system explanation
...
- Batch cooking recipes must automatically scale to the family's profile (size × days)
- The shopping list must always show an estimated total vs. the family's weekly budget
